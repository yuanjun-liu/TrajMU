...
from _tool.mList import bisearch_lambda_list, choice
def to_item_th(x):
    if isinstance(x, list) or isinstance(x, np.ndarray) or isinstance(x, torch.Tensor):
        return [to_item_th(i) for i in x]
    if isinstance(x, tuple):
        return (to_item_th(i) for i in x)
    if isinstance(x, dict):
        return {k: to_item_th(x[k]) for k in x}
    try:
        return x.item()
    except:
        return x
...
def detach(x):
    if isinstance(x, Tensor):
        return x.detach()
    if isinstance(x, list):
        return [detach(i) for i in x]
    if isinstance(x, tuple):
        return tuple([detach(i) for i in x])
    if isinstance(x, set):
        return {detach(i) for i in x}
    if isinstance(x, dict):
        return {k: detach(x[k]) for k in x}
    return x

def call_estop_fn(num):
    """stop if loss don't decrease in num steps"""
    cur_step = 0
    best_step, best_loss = 0, np.inf
    def estop_fn(loss):
        nonlocal cur_step, best_step, best_loss
        if loss < best_loss:
            best_step = cur_step
            best_loss = loss
        cur_step += 1
        return cur_step - best_step > num
    return estop_fn
def basic_train(
    model: Module,
    ...
    log_step=10,
    sch=None,
    train_after_batch_fn=None,
    train_after_epoch_fn=None,
    train_before_train_fn=None,
    estop_fn=None,
    save_ep_fn=None,
    **kw,
):
    """call(y,x)"""
    if train_before_train_fn is not None:
        train_before_train_fn()
    model.train()
    finish = False
    for ep in range(epoch_max):
        for i, x in enumerate(train_loader):
            ...
            if log and i % log_step == 0:
                print(f'ep:{ep},step:{i},loss:{loss.item()}')
            if torch.isnan(loss):
        ...
        if train_after_epoch_fn is not None and train_after_epoch_fn():
            ...
            break
    if path:
        save_weight(model, path, opt=opt, sch=sch)
    return model
