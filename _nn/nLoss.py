...
def loss_reduce(loss:Tensor,reduction='mean'):
    ...
    if isinstance(reduction,str): 
        ...
        elif reduction=='abssum':return loss.abs().sum() 
        elif reduction=='max':return loss.max() 
        elif reduction=='min':return loss.min() 
        elif reduction=='norm':return torch.norm(loss,2)
    return loss
